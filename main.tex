...
\newcommand{\cvresearch}[3]{%
  \par\Needspace{5\baselineskip}\textbf{#1}\par
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}X >{\raggedleft\arraybackslash}p{0.32\textwidth}@{}}
...
\cvresearch{AirMiniVLA: Memory- and Stage-Aware Vision-Language Navigation}{UAV vision-language navigation}{Feb 2026 -- present}
\begin{cvitems}
\item Proposed an efficient navigation framework combining compact temporal memory with instruction-stage alignment for history-aware action prediction.
\item The approximately 1B-parameter framework achieved up to 50\% relative improvement over existing baselines.
\end{cvitems}
\cventrygap
